...
export default function ExperimentExplainer({ experiment }: ExperimentExplainerProps) {
  const [isExpanded, setIsExpanded] = useState(false);
  const [markdownContent, setMarkdownContent] = useState<string>('');
  const [isLoading, setIsLoading] = useState(false);

  useEffect(() => {
    if (isExpanded && experiment.markdownFile) {
      loadMarkdownContent();
    }
  }, [isExpanded, experiment.markdownFile]);
...
[Content will be added when this experiment is implemented]`,
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg overflow-hidden">
      ...
      >
        <h2 className="text-xl font-bold text-emerald-600 dark:text-emerald-300">
